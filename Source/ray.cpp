...
{
    glm::vec3 outgoing = glm::normalize(glm::vec3(start.coordinates) - glm::vec3(end.coordinates));
    glm::vec3 normal = glm::vec3(vert.v_normal->vectorCoordinates);

    //std::cout << outgoing.x << " " << outgoing.y << " " << outgoing.z << std::endl;
    //std::cout << normal.x << " " << normal.y << " " << normal.z << std::endl;

    glm::vec4 outgoingRay = glm::vec4(outgoing, 1.0f);
    float angle = glm::angle(glm::vec3(outgoingRay), glm::vec3(vert.v_normal->vectorCoordinates));
    return cosf(angle);
